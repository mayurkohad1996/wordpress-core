...
class WP_Debug_Data {
	...
	private static function get_wp_server(): array {
		...
		if ( is_file( ABSPATH . 'robots.txt' ) ) {
			...
			$robotstxt_string = sprintf( __( 'There is a physical %s file in your installation folder. WordPress is not using a virtual generated one.' ), 'robots.txt' );
		} else {
			// No robots.txt file available, turn debug info to false.
			$robotstxt_debug = false;

			/* translators: %s: robots.txt */
			$robotstxt_string = sprintf( __( 'Your site is using the virtual %s file which is generated by WordPress.' ), 'robots.txt' );
		}
		$info['wp-server']['fields']['physical_robotstxt_file'] = array(
			'label' => __( 'Is there a physical robots.txt file available?' ),
